...
API_ENDPOINT = "https://create.fandom.com/api.php"
DEFAULT_USER_AGENT = "MC_Agent/0.1 (Create Wiki crawler; D:/magic/MC_Agent)"

CORE_TITLES = [
    "Create Mod",
    "Tutorials/Tree Automation",
    "Mechanical Press",
    "Mechanical Mixer",
    "Millstone",
    "Crushing Wheel",
...
def request_json(params: dict[str, Any], user_agent: str, retries: int = 2) -> dict[str, Any]:
    ...
    raise RuntimeError(f"Create Wiki API request failed: {last_error}")
...
def seed_titles_for_query(query: str) -> list[str]:
    lowered = query.lower()
    titles: list[str] = []
    if any(token in lowered for token in ("create", "mechanical", "automation", "机械动力", "自动化", "好玩", "玩法")):
        titles.extend(CORE_TITLES)
    if any(token in lowered for token in ("train", "铁路", "火车")):
        titles.extend(["Train", "Track", "Train Station", "Train Schedule"])
    if any(token in lowered for token in ("steam", "蒸汽")):
        titles.extend(["Steam Engine", "Steam Whistle"])
    return titles
...
def page_to_markdown(page: dict[str, Any], fetched_at: str, query: str, content: str) -> str:
    ...
    url = str(page.get("fullurl") or f"https://create.fandom.com/wiki/{title.replace(' ', '_')}")
    lines = [
        f"# {title}",
        "",
        "<!-- source: createwiki_mediawiki_api -->",
        "",
        "## Metadata",
        "",
        f"- **Create Wiki URL:** {url}",
...
def fetch_seed(dest_root: Path, query: str, search_limit: int, user_agent: str, force: bool = False) -> dict[str, Any]:
    run_dir = dest_root / "createwiki" / datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    ...
    for page in pages:
        ...
        url = str(page.get("fullurl") or f"https://create.fandom.com/wiki/{title.replace(' ', '_')}")
        key = make_key("createwiki", item_id)
        ...
        if previous and previous.get("content_hash") == digest and not force:
            ...
            continue
        path = run_dir / f"create_{slugify(title)}.md"
    ...
    manifest = {
        "manifest_type": "createwiki_seed_export",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fetch Create Wiki pages as Markdown for MCagent RAG.")
    parser.add_argument("--dest", default=str(PROJECT_ROOT / "data" / "crawler_exports"))
    parser.add_argument("--query", default="Create mod automation")
