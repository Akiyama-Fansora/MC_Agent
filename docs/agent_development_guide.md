...
## 58. CrawlerJobSetupService：Crawler job 启动编排服务化（2026-05-22）

本轮开始前已重新阅读本文档。第 57 阶段拆出了运行中状态 payload，本轮继续处理 `_run_crawler_job()` 最外层启动逻辑：planner source 判断、单来源任务构造、fallback plan、停止阶段 payload 和循环限制值。

### 58.1 本轮目标

把启动编排中“客观、可验证、无主观决策”的部分抽成服务：

- `is_planner_source()`：只判断 source 是否属于 CrawlerAgent planner 类入口；
- `single_source_tasks()`：把明确指定来源的请求转成单个工具任务；
- `fallback_plan()`：在 LLM planner 无任务时承接 `_all_source_tasks()` 的客观结果；
- `limits()`：计算 max_replans、initial_task_limit、max_total_tasks；
- `stopped_update()`：统一规划前、规划中、规划后的 stopped job payload。

这个服务不搜索、不调用 LLM、不决定目标、不替 CrawlerAgent 选择下一步。它只是把启动阶段的固定合同收拢，让 `_run_crawler_job()` 更接近“Agent loop + 工具执行器”的结构。

### 58.2 代码变更

1. 新增 `mcagent/crawler_job_setup_service.py`：
   - 提供 planner source、单来源任务、fallback plan、限制值、停止 payload 的纯函数式服务。
2. 修改 `mcagent/web_server.py`：
   - `_run_crawler_job()` 入口实例化 setup/progress/runtime/accounting/finalization 等服务；
   - 修复 `job_progress` 在 planner 分支使用前尚未实例化的问题；
   - 用 `CrawlerJobSetupService` 替换散落的 planner source 集合判断、单来源任务内联字典、fallback plan 字典和限制值计算。
3. 新增 `tests/crawler_job_setup_service_scenarios.py`：
   - 覆盖 planner source、query fallback、fallback plan、limits cap、stopped payload。
4. 更新 `.github/workflows/ci.yml`、`README.md`、`scripts/public_readiness_check.py`。

### 58.3 边界说明

`CrawlerJobSetupService` 不是 planner，也不是策略层。它不能根据用户语义修改目标，不能把“要不要补库”“搜什么关键词”“下一步执行哪个来源”写死。真正的任务理解仍由 MCagent/CrawlerAgent 的 LLM router、planner 和 reflection loop 完成。
...
公开检查仍只有 LICENSE 非阻断警告。第一次本地 `py_compile` 遇到旧 `__pycache__` 写入权限冲突，已用 `PYTHONPYCACHEPREFIX` 指向临时缓存目录重跑通过；CI 的干净工作区不受影响。

### 58.5 下一步

继续收敛 `_run_crawler_job()` 的剩余大块职责：优先考虑把 planner 超时轮询 `_plan_crawler_with_job_timeout()` 的状态 payload 与规划执行隔离，或者把 replan/topic discovery 的 LLM materialization 进一步拆成明确的 Crawler planning adapter。

## 59. CrawlerPlannerWaitService：Crawler planner 等待状态服务化（2026-05-22）

本轮开始前已重新阅读本文档。第 58 阶段拆出了 job 启动编排，本轮继续处理 `_plan_crawler_with_job_timeout()`：它仍然保留异步等待 LLM planner 的职责，但不再内联构造“正在理解任务”的 UI/API payload。

### 59.1 本轮目标

把 Crawler planner 等待阶段的客观状态拆成服务：

- `context()`：从 session summary 里提取展示用 `planner_topic`、`handoff_brief`、`delivery_target`；
- `stopped_plan()`：在用户请求停止且 planner 尚未完成时生成 stopped plan 合同；
- `waiting_update()`：构造 planner 等待中的 summary/result/loop payload。

这些逻辑只负责展示与合同格式，不调用 LLM，不生成采集任务，也不判断下一步采集来源。

### 59.2 代码变更

1. 新增 `mcagent/crawler_planner_wait_service.py`：
   - 封装 planner 等待阶段的上下文提取、停止 plan 和 running payload。
2. 修改 `mcagent/web_server.py`：
   - `_plan_crawler_with_job_timeout()` 使用 `CrawlerPlannerWaitService`；
   - 删除函数内联 session summary 提取和等待 payload 大字典。
3. 新增 `tests/crawler_planner_wait_service_scenarios.py`：
   - 覆盖目标提取优先级、无 summary fallback、stopped plan、waiting loop payload。
4. 更新 `.github/workflows/ci.yml`、`README.md`、`scripts/public_readiness_check.py`。

### 59.3 边界说明

`CrawlerPlannerWaitService` 不拥有“规划”。它只是等待期状态构造器。真正的规划仍在 `plan_crawler_tasks_resilient()` 以及 CrawlerAgent 的后续 reflection loop 中完成；如果 planner 超时期间需要给用户状态反馈，只展示“还在理解/规划”，不伪造任务、不替 LLM 选择搜索词。
...
## 2026-06-10 Stage 78: Unified AgentMessage Route Tool

Stage 78 fixes a root capability gap: Agents had background collection handoff tools, but no single no-persistence route tool for ordinary From-Content-To communication between User, MCagent, and CrawlerAgent.

Without that capability, MCagent could incorrectly answer on another Agent's behalf, or overfit a normal message into `delegate_crawler` and risk a collection side effect. This is not fixed by adding sentence-specific routing. The fix is to expose one general `agent_message` capability to the LLM and require the LLM to semantically choose the recipient.

Implemented changes:

1. Added route tool `agent_message` to both MCagent and CrawlerAgent route catalogs.
2. `agent_message` sends a normal `AgentMessage(from_agent=..., content=..., to_agent=...)` with optional semantic intent.
3. The receiving Agent still owns its route decision and may choose any allowed route from its own catalog.
4. `agent_message` is explicitly no-persistence and does not start background jobs.
5. `delegate_crawler` remains the separate tool for collection, saving, ingest,补库, or background-job side effects.
6. The runtime no longer uses a pre-route Crawler relay shortcut or Crawler-specific ordinary-message tool.
7. Added tests proving MCagent can message CrawlerAgent without creating a job and without relying on a fixed phrase.

Boundary:

The runtime still does not infer the user's intent from hardcoded test phrases. The active Agent's LLM chooses between `direct_answer`, `agent_message`, `delegate_crawler`, local RAG/context, and other tools from its catalog. The message bus only delivers the chosen message; it does not decide the receiver's answer or tool route.
