...
def test_conversation_graph_routes_only_by_message_target() -> None:
    ...
    assert_true("mcagent_route_decision_output_kind", route_decision_output.get("contract_kind") == "mcagent_route_decision_output_facts_contract", str(route_decision_output))
    assert_true("mcagent_route_decision_output_owner", route_decision_output.get("decision_owner") == "MCagent LLM", str(route_decision_output))
    assert_true("mcagent_route_decision_output_links_runtime", route_decision_output.get("runtime_request_id") == runtime_request.get("request_id"), str(route_decision_output))
    assert_true("mcagent_route_decision_output_links_route_input", route_decision_output.get("route_input_contract_id") == route_input.get("contract_id"), str(route_decision_output))
    assert_true("mcagent_route_decision_output_links_message_preflight", route_decision_output.get("message_preflight_contract_id") == message_preflight.get("contract_id"), str(route_decision_output))
    assert_true("mcagent_route_decision_output_links_contextual_question", route_decision_output.get("contextual_question_contract_id") == contextual_question.get("contract_id"), str(route_decision_output))
    assert_true("mcagent_route_decision_output_no_fake_trace", route_decision_facts.get("has_tool_selected_trace") is False, str(route_decision_output))
...
def test_conversation_graph_can_dispatch_to_crawler_node() -> None:
    ...
    route_decision_output = agent_runtime.get("route_decision_output_contract") or {}
    route_decision_facts = route_decision_output.get("trace_facts") or {}
    assert_true("crawler_route_decision_output_kind", route_decision_output.get("contract_kind") == "crawler_route_decision_output_facts_contract", str(route_decision_output))
    assert_true("crawler_route_decision_output_owner", route_decision_output.get("decision_owner") == "CrawlerAgent LLM", str(route_decision_output))
    assert_true("crawler_route_decision_output_links_runtime", route_decision_output.get("runtime_request_id") == runtime_request.get("request_id"), str(route_decision_output))
    assert_true("crawler_route_decision_output_links_route_input", route_decision_output.get("route_input_contract_id") == route_input.get("contract_id"), str(route_decision_output))
    assert_true("crawler_route_decision_output_links_message_preflight", route_decision_output.get("message_preflight_contract_id") == message_preflight.get("contract_id"), str(route_decision_output))
    assert_true("crawler_route_decision_output_links_source_planning", route_decision_output.get("source_planning_contract_id") == source_planning.get("contract_id"), str(route_decision_output))
    assert_true("crawler_route_decision_output_links_side_effect_auth", route_decision_output.get("side_effect_authorization_contract_id") == side_effect_auth.get("contract_id"), str(route_decision_output))
    assert_true("crawler_route_decision_output_no_fake_trace", route_decision_facts.get("has_tool_selected_trace") is False, str(route_decision_output))
...
def test_graph_direct_answer_node_executes_for_both_agents() -> None:
    deliveries: list[str] = []
    executions: list[dict[str, str]] = []
    ...
    def route_decider(
        ...
        *,
        ...
        return {
            "route_decision_id": f"{payload.get('session_id')}:{agent_id}:direct_answer",
            "node": node_name,
            "graph": graph_name,
            "agent_id": agent_id,
            "agent": agent_id,
            ...
            "decision_owner": "CrawlerAgent LLM" if agent_id == "crawler_agent" else "MCagent LLM",
            "legacy_fallback_required": False,
            "tool_decision": {"tool": "direct_answer", "reason": f"{agent_id} can answer directly"},
            "route_confirmation": {"proceed": True, "tool": "direct_answer", "reason": "confirmed"},
            "action_plan": [],
            "trace": [{"stage": "decide", "status": "tool_selected", "detail": {"tool": "direct_answer"}}],
        }

    def direct_answer_executor(
        config: AppConfig,
        payload: dict[str, Any],
        *,
        emit: Any | None = None,
        ...
        executions.append({"agent": agent_id, "node": node_name})
        return {
            "answer": f"{agent_id} direct answer",
            "agent": agent_id,
            "sources": [],
            "context": "",
            "trace": [{"stage": "answer", "status": "generating", "detail": {"model": "fake", "mode": "direct"}}],
            "metadata": {
                "graph_route_decision": {
                    "routed": True,
                    "route_decision_id": route_decision.get("route_decision_id") or "",
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        mc_result = dispatch_agent_message_graph(
            config,
            {"session_id": "runtime-direct-answer-mc"},
            from_agent="User",
            content="hello mc",
            to_agent="MCagent",
            conversation_id="runtime-direct-answer-mc",
            agent_delivery=legacy,
            route_decider=route_decider,
        ...
        crawler_result = dispatch_agent_message_graph(
            config,
            {"session_id": "runtime-direct-answer-crawler"},
            from_agent="User",
            content="hello crawler",
            to_agent="CrawlerAgent",
            conversation_id="runtime-direct-answer-crawler",
            agent_delivery=legacy,
            route_decider=route_decider,
            direct_answer_executor=direct_answer_executor,
        )
    ...
    assert_true("legacy_not_called", deliveries == [], str(deliveries))
    assert_true("both_direct_answer_executed", executions == [
        {"agent": "mcagent_rag", "node": "mcagent.graph_direct_answer_node"},
        {"agent": "crawler_agent", "node": "crawler.graph_direct_answer_node"},
    ], str(executions))
    mc_runtime = mc_result.get("agent_graph_runtime") or {}
    crawler_runtime = crawler_result.get("agent_graph_runtime") or {}
    assert_true("mc_graph_direct_answer_node", "mcagent.graph_direct_answer_node" in mc_runtime.get("visited_nodes", []), str(mc_runtime))
    assert_true("crawler_graph_direct_answer_node", "crawler.graph_direct_answer_node" in crawler_runtime.get("visited_nodes", []), str(crawler_runtime))
    assert_true("mc_direct_answer_legacy_not_visited", "mcagent.legacy_adapter" not in mc_runtime.get("visited_nodes", []), str(mc_runtime))
    assert_true("crawler_direct_answer_legacy_not_visited", "crawler.legacy_adapter" not in crawler_runtime.get("visited_nodes", []), str(crawler_runtime))
    assert_true("mc_direct_answer_adapter", (mc_runtime.get("runtime_adapter") or {}).get("adapter") == "graph_direct_answer_node_executor", str(mc_runtime))
    assert_true("crawler_direct_answer_adapter", (crawler_runtime.get("runtime_adapter") or {}).get("adapter") == "graph_direct_answer_node_executor", str(crawler_runtime))
    assert_true("mc_direct_answer_execution_contract", (mc_runtime.get("route_execution_contract") or {}).get("route_execution_executed_by_graph") is True, str(mc_runtime))
    assert_true("crawler_direct_answer_execution_contract", (crawler_runtime.get("route_execution_contract") or {}).get("route_execution_executed_by_graph") is True, str(crawler_runtime))
    assert_true("mc_direct_answer_conversation_node", "mcagent_graph.graph_direct_answer_node" in (mc_result.get("graph_runtime") or {}).get("visited_nodes", []), str(mc_result))
