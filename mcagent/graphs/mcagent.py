...
def build_mcagent_graph(
    ...
    *,
    ...
    def run_graph_local_corpus_inventory_route(state: AgentGraphState) -> dict[str, Any]:
        runtime_request = state.get("runtime_request") if isinstance(state.get("runtime_request"), dict) else {}
        route_decision = state.get("route_decision") if isinstance(state.get("route_decision"), dict) else {}
        result = dict(
            local_corpus_inventory_executor(
                config,
                dict(state.get("payload") or {}),
                emit=emit,
                agent_id="mcagent_rag",
                graph_name="MCagentGraph",
                node_name="mcagent.graph_local_corpus_inventory_route",
                runtime_request=runtime_request,
                route_decision=route_decision,
            )
        )
        adapter = graph_local_corpus_inventory_route_executor_metadata(
            agent_id="mcagent_rag",
            graph_name="MCagentGraph",
            node_name="mcagent.graph_local_corpus_inventory_route",
            runtime_request=runtime_request,
        ...
        return {
            ...
            **_append(
                state,
                "mcagent.graph_local_corpus_inventory_route",
                "executed_agent_selected_local_corpus_inventory",
                {
                    "agent": "mcagent_rag",
    ...
    def run_graph_router_error_route(state: AgentGraphState) -> dict[str, Any]:
        ...
        route_decision = state.get("route_decision") if isinstance(state.get("route_decision"), dict) else {}
        result = dict(
            router_error_executor(
                config,
                dict(state.get("payload") or {}),
                emit=emit,
                agent_id="mcagent_rag",
                graph_name="MCagentGraph",
                node_name="mcagent.graph_router_error_route",
                runtime_request=runtime_request,
                route_decision=route_decision,
            )
        )
        adapter = graph_router_error_route_executor_metadata(
            agent_id="mcagent_rag",
            graph_name="MCagentGraph",
            node_name="mcagent.graph_router_error_route",
            runtime_request=runtime_request,
        ...
        return {
            ...
            **_append(
                state,
                "mcagent.graph_router_error_route",
                "executed_agent_selected_router_error",
                {
                    "agent": "mcagent_rag",
    ...
    def run_graph_direct_answer_node(state: AgentGraphState) -> dict[str, Any]:
        ...
        result = dict(
            direct_answer_executor(
                ...
                agent_id="mcagent_rag",
                graph_name="MCagentGraph",
                node_name="mcagent.graph_direct_answer_node",
                runtime_request=runtime_request,
                route_decision=route_decision,
            )
        )
        ...
        return {
            ...
            **_append(
                state,
                "mcagent.graph_direct_answer_node",
    ...
    def run_graph_temporary_extract_node(state: AgentGraphState) -> dict[str, Any]:
        ...
        result = dict(
            temporary_extract_executor(
                ...
                agent_id="mcagent_rag",
                graph_name="MCagentGraph",
                node_name="mcagent.graph_temporary_extract_node",
                runtime_request=runtime_request,
                route_decision=route_decision,
            )
        )
        ...
        return {
            ...
            **_append(
                state,
                "mcagent.graph_temporary_extract_node",
    ...
    def route_execution_key(state: AgentGraphState) -> str:
        ...
        allows_execution = graph_route_decision_allows_execution(decision)
        if status_executor is not None and decision.get("routed") and decision.get("route_intent") == "status" and allows_execution:
            return "status"
        if crawler_audit_executor is not None and decision.get("routed") and decision.get("route_intent") == "crawler_audit" and allows_execution:
