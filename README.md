...
- `AgentMessage`：统一消息通道，所有用户与 Agent、Agent 与 Agent 的沟通都可表示为 `(from_agent, content, to_agent)`；消息通道只负责送达，接收方 Agent 自己决定下一步工具。
...
- CrawlerAgent：支持 MC百科、Modrinth、本地 HTTP URL 抓取、公开搜索发现、Playwright/browser_collect、本地文件读写搜索、整合包包体解析和通用保存工具。
...
- 结构化消息接口：`POST /api/agent-message`，用于显式发送 `User -> MCagent`、`User -> CrawlerAgent` 或 Agent 间消息。
- Agent 回复也会返回 `agent_message`，例如 `MCagent -> User`；SSE 仍保留 `trace`、`delta`、`response`、`done` 事件。
...
- 回归测试矩阵见 `docs/agent_test_matrix.md`，以后修改两个 Agent 都要覆盖五类测试：MCagent 本地问答、MCagent 缺口委托、Crawler 临时网页读取、Crawler 补库给 RAG、Crawler 保存到用户指定目录。
...
python scripts\audit_dual_agent_architecture.py
